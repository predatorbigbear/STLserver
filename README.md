...
但是目前发现还存在两个影响巨大的bug，第一个就是实测发现进入http的处理类在执行redis和mysql操作后只有60%的能执行回收处理，这个是通过对每个类对象传递一个标志数字，然后在setready和clean中分别记录下来发现的，这个bug导致在回收过程全部执行完毕后再次进行测试会出现问题。
第二个问题就是日志库在写入超过缓冲区大小的数据后后面的数据好像记录不进去了。
目前优先修复第一个bug，修复好后再投递简历。要进行大范围性能测试的话目前只能进行一次,可以使用接口4或者接口8进行对比，接口8是带body参数解析版本 
有bug，注意！
有bug，注意！
有bug，注意！
