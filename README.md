崩溃原因很简单，一开始以为是越界，用工具测试也是越界，但是后来检查了越界提示的位置，发现并没有超越分配大小，后来最终发现是内存池中的new 类型后面跟着的不是[而是(  ，这个原因主要是之前的实现是void类型，使用的是malloc，后来改成char[]的时候只是简单改写了new char，没有把(改掉，另外一个问题就是fastSafeList中的unique_ptr类型是T而不是T[]，这两处都修复了，经过测试现在不会崩溃了
