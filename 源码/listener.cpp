...
void listener::startAccept()
{
	try
	{
		...
		{
			//m_log->writeLog(__FUNCTION__, __LINE__, "  notifySocketPool  ");
			notifySocketPool();
		}
	}
	catch (const std::exception &e)
	{
		//m_log->writeLog(__FUNCTION__, __LINE__, e.what());
		restartAccept();
	}
}
...
void listener::handleStartAccept(std::shared_ptr<HTTPSERVICE> httpServiceTemp, const boost::system::error_code &err)
{
	...
	else
	{
		if (httpServiceTemp)
		{
			...
			{
				//m_log->writeLog(__FUNCTION__, __LINE__, " setReady ");
				httpServiceTemp->setReady(temp, m_clearFunction, httpServiceTemp);
			}
			else
			{
				//m_log->writeLog(__FUNCTION__, __LINE__, " getBackElem ");
