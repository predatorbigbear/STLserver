...
LOG::LOG(const char *logFileName, std::shared_ptr<IOcontextPool> ioPool, bool &result ,const int overTime , const int bufferSize ):m_overTime(overTime),m_bufferSize(bufferSize)
{
	try
	{
		...
		m_timer.reset(new steady_timer(*ioPool->getIoNext()));

		std::error_code err{};
		if (fs::exists(logFileName,err))
		{
			m_file.open(logFileName, std::ios::trunc);
			if (!m_file)
		...
		if (!file)
		{
			std::ofstream fileTemp(logFileName, std::ios::out | std::ios::app);
			if (!fileTemp)
				throw std::runtime_error(string(logFileName) + " can not create");
		}
...
{
	m_timer->expires_from_now(std::chrono::seconds(m_overTime));
