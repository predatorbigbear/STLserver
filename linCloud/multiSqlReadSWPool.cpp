...
MULTISQLREADSWPOOL::MULTISQLREADSWPOOL(std::shared_ptr<IOcontextPool> ioPool, std::shared_ptr<function<void()>> unlockFun, const std::string & SQLHOST,
...
std::shared_ptr<MULTISQLREADSW> MULTISQLREADSWPOOL::getSqlNext()
{
	m_sqlMutex.lock();
...
void MULTISQLREADSWPOOL::readyReadyList()
{
	try
	{
		if (m_bufferNum)
...
void MULTISQLREADSWPOOL::nextReady()
{
	--m_bufferNum;
	readyReadyList();
}
