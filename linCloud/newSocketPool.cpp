#include "newSocketPool.h"

NewSocketPool::NewSocketPool(std::shared_ptr<IOcontextPool> ioPool, std::shared_ptr<std::function<void()>> startFunction, 
...
void NewSocketPool::ready(int beginSize)
{
	try
	{
		if (!m_bufferList)
...
void NewSocketPool::getNextBuffer(std::shared_ptr<ReadBuffer>& outBuffer)
{
...
void NewSocketPool::getBackElem(const std::shared_ptr<ReadBuffer> &buffer)
{
...
void NewSocketPool::setNotifyAccept()
{
