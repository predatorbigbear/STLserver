#include "sqlReadPool.h"

SQLREADPOOL::SQLREADPOOL(std::shared_ptr<IOcontextPool> ioPool, std::shared_ptr<std::function<void()>> unlockFun, const std::string & SQLHOST, const std::string & SQLUSER,
	const std::string & SQLPASSWORD,
	...
	m_SQLDB(SQLDB),m_SQLPORT(SQLPORT), m_ioPool(ioPool)
{
	try
	{
		...
		m_loop.reset(new std::function<void()>(std::bind(&SQLREADPOOL::nextReady, this)));
		readyReadyList();
	}
...
std::shared_ptr<SQLREAD> SQLREADPOOL::getSqlNext()
{
...
void SQLREADPOOL::readyReadyList()
{
	try
	{
		if (m_bufferNum)
		{
			//std::shared_ptr<SQLREAD>tempSql{ std::make_shared<SQLREAD>(m_ioPool->getIoNext(),m_loop , m_SQLHOST, m_SQLUSER, m_SQLPASSWORD, m_SQLDB, m_SQLPORT) };
			m_sqlPool.emplace_back(std::make_shared<SQLREAD>(m_ioPool->getIoNext(), m_loop, m_SQLHOST, m_SQLUSER, m_SQLPASSWORD, m_SQLDB, m_SQLPORT));
		}
		else
		{
			(*m_unlockFun)();
