                ...
                else
                {
                    ...
                    while (strBegin != strEnd)
                    {
                        ...
                        case MYSQL_TYPE_GEOMETRY:
                            ...
                            //据MySQL 8.0规范，SET类型字符串的长度限制如下：

                            //最大成员数量
                            //    ‌成员数量上限‌：SET类型最多支持64个成员（即64个不同的值）。
                            //    单个成员长度
                            //    ‌单个成员最大长度‌：每个成员（值）的最大长度为255字符（字节）。
                            //    总字符串长度
                            //    ‌总长度计算‌：SET类型字段的总字符串长度取决于成员数量和每个成员的长度。例如：
                            //    64个成员，每个成员255字符：总长度 = 64 * 255 = 16, 320字符。
                            //    实际使用时，通常成员数量远少于64，且每个成员长度也小于255。
                            //    默认行为‌：如果SET类型字段未声明为NOT NULL，则可以插入NULL值
                        case MYSQL_TYPE_STRING:
                            ...
                            else
                            {
                                ...
                                {
                                    //252
                                    //当长度为252时  strBegin+1与strBegin+2为正确长度  strBegin+3为数据开始位置 
