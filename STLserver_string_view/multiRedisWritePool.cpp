...
MULTIREDISWRITEPOOL::MULTIREDISWRITEPOOL(std::shared_ptr<IOcontextPool> ioPool, std::shared_ptr<std::function<void()>> unlockFun, const std::string & redisIP, 
	const unsigned int redisPort, const unsigned int bufferNum, const unsigned int memorySize, const unsigned int outRangeMaxSize, const unsigned int commandSize)
...
void MULTIREDISWRITEPOOL::readyReadyList()
{
	...
	{
		m_redisMutex.lock();
		m_redisList.emplace_back(std::make_shared<MULTIREDISWRITE>(m_ioPool->getIoNext(), m_innerUnLock, m_redisIP, m_redisPort, m_memorySize, m_outRangeMaxSize, m_commandSize));
	}
	m_redisMutex.lock();
	unlock();
	m_redisMutex.unlock();
