...
FixedWEBSERVICEPOOL::FixedWEBSERVICEPOOL(std::shared_ptr<IOcontextPool> ioPool, const std::string &doc_root,
...
bool FixedWEBSERVICEPOOL::ready()
{
	try
	{
		m_iterEnd = nullptr;
		if (!m_bufferList)
		{
			m_bufferList.reset(new std::shared_ptr<WEBSERVICE>[m_beginSize]);
...
void FixedWEBSERVICEPOOL::getNextBuffer(std::shared_ptr<WEBSERVICE>& outBuffer)
{
...
void FixedWEBSERVICEPOOL::getBackElem(std::shared_ptr<WEBSERVICE>& buffer)
{
...
void FixedWEBSERVICEPOOL::setNotifyAccept()
{
	m_listMutex.lock();
	m_startAccept = false;
	m_listMutex.unlock();
