...
struct MULTIREDISREADCOPYPOOL
{
	MULTIREDISREADCOPYPOOL(std::shared_ptr<IOcontextPool> ioPool, std::shared_ptr<LOGPOOL> logPool, std::shared_ptr<std::function<void()>>unlockFun,
	...
	std::shared_ptr<MULTIREDISREADCOPY> getRedisNext();
...
private:
	...
	std::vector<std::shared_ptr<MULTIREDISREADCOPY>> m_redisList;
	std::vector<std::shared_ptr<MULTIREDISREADCOPY>>::iterator m_iterNow;
	...
	std::shared_ptr<MULTIREDISREADCOPY> m_redis_temp{};
