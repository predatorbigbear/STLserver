...
MULTIREDISREADCOPYPOOL::MULTIREDISREADCOPYPOOL(std::shared_ptr<IOcontextPool> ioPool, std::shared_ptr<LOGPOOL> logPool, std::shared_ptr<std::function<void()>> unlockFun, 
	...
	m_memorySize(memorySize),m_outRangeMaxSize(outRangeMaxSize),m_commandSize(commandSize), m_logPool(logPool),m_timeWheel(timeWheel)
{
	...
	m_innerUnLock.reset(new std::function<void()>(std::bind(&MULTIREDISREADCOPYPOOL::innerUnlock, this)));
...
std::shared_ptr<MULTIREDISREADCOPY> MULTIREDISREADCOPYPOOL::getRedisNext()
{
	m_redisMutex.lock();
...
void MULTIREDISREADCOPYPOOL::readyReadyList()
{
	for (unsigned int i = 0; i != m_bufferNum; ++i)
	{
		m_redisMutex.lock();
		m_redisList.emplace_back(std::make_shared<MULTIREDISREADCOPY>(m_ioPool->getIoNext(), m_logPool->getLogNext(), m_innerUnLock,
			m_timeWheel,
...
void MULTIREDISREADCOPYPOOL::unlock()
{
	(*m_unlockFun)();
}


void MULTIREDISREADCOPYPOOL::innerUnlock()
{
	m_redisMutex.unlock();
}
