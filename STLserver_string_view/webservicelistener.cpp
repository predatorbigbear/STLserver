...
WEBSERVICELISTENER::WEBSERVICELISTENER(std::shared_ptr<IOcontextPool> ioPool,
	...
	m_multiSqlWriteSWPoolMaster(multiSqlWriteSWPoolMaster), m_randomCode(randomCode)
{
	if (m_ioPool &&  !doc_root.empty() && !tcpAddress.empty() && logPool && m_timeOut > 0 && checkSecond && m_timeWheel
		&& m_multiSqlReadSWPoolMaster && m_fileVec && m_BGfileVec && m_randomCode
		...
		)
	{
		...
		m_startFunction.reset(new std::function<void()>(std::bind(&WEBSERVICELISTENER::reAccept, this)));
		m_clearFunction.reset(new std::function<void(std::shared_ptr<WEBSERVICE>&)>(std::bind(&WEBSERVICELISTENER::getBackHTTPSERVICE, this, std::placeholders::_1)));
		...
		m_startcheckTime.reset(new std::function<void()>(std::bind(&WEBSERVICELISTENER::checkTimeOut, this)));
		m_httpServiceList.reset(new FIXEDTEMPLATESAFELIST<std::shared_ptr<WEBSERVICE>>(m_startcheckTime, m_socketNum));
...
void WEBSERVICELISTENER::resetEndpoint()
{
	try
	{
...
void WEBSERVICELISTENER::resetAcceptor()
{
	try
	{
...
void WEBSERVICELISTENER::openAcceptor()
{
	error_code err;
...
void WEBSERVICELISTENER::bindAcceptor()
{
	error_code err;
...
void WEBSERVICELISTENER::listenAcceptor()
{
	error_code err;
...
void WEBSERVICELISTENER::startAccept()
{
	try
	{
		std::shared_ptr<WEBSERVICE> httpServiceTemp{  };
...
void WEBSERVICELISTENER::handleStartAccept(std::shared_ptr<WEBSERVICE> httpServiceTemp, const boost::system::error_code &err)
{
...
void WEBSERVICELISTENER::getBackHTTPSERVICE(std::shared_ptr<WEBSERVICE> &tempHs)
{
	if (tempHs )
	{
...
void WEBSERVICELISTENER::startRun()
{
...
void WEBSERVICELISTENER::checkTimeOut()
{
