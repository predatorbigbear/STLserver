...
struct FixedHTTPSSERVICEPOOL
{
	FixedHTTPSSERVICEPOOL(std::shared_ptr<IOcontextPool> ioPool, const std::string &doc_root,
	...
	void getNextBuffer(std::shared_ptr<HTTPSSERVICE> &outBuffer);

	void getBackElem(std::shared_ptr<HTTPSSERVICE> &buffer);
...
private:
	const std::shared_ptr<std::function<void(std::shared_ptr<HTTPSSERVICE>&)>>  m_cleanFun{};
	...
	std::unique_ptr<std::shared_ptr<HTTPSSERVICE>[]> m_bufferList{};
	std::shared_ptr<HTTPSSERVICE> *m_iterNow{};
	std::shared_ptr<HTTPSSERVICE> *m_iterEnd{};
	...
	std::shared_ptr<HTTPSSERVICE> m_temp{};
