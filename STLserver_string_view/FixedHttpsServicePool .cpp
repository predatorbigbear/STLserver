...
FixedHTTPSSERVICEPOOL::FixedHTTPSSERVICEPOOL(std::shared_ptr<IOcontextPool> ioPool, const std::string &doc_root,
	...
	const unsigned int timeOut, const std::shared_ptr<std::function<void(std::shared_ptr<HTTPSSERVICE>&)>> &cleanFun,
	int beginSize):
...
bool FixedHTTPSSERVICEPOOL::ready()
{
	try
	{
		m_iterEnd = nullptr;
		if (!m_bufferList)
		{
			m_bufferList.reset(new std::shared_ptr<HTTPSSERVICE>[m_beginSize]);
			m_iterNow = m_bufferList.get();
			for (i = 0; i != m_beginSize; ++i)
			{
				m_success = true;
				*m_iterNow++ = std::make_shared<HTTPSSERVICE>(m_ioPool->getIoNext(), m_logPool->getLogNext(), m_doc_root,
...
void FixedHTTPSSERVICEPOOL::getNextBuffer(std::shared_ptr<HTTPSSERVICE>& outBuffer)
{
...
void FixedHTTPSSERVICEPOOL::getBackElem(std::shared_ptr<HTTPSSERVICE>& buffer)
{
...
void FixedHTTPSSERVICEPOOL::setNotifyAccept()
{
	m_listMutex.lock();
	m_startAccept = false;
	m_listMutex.unlock();
