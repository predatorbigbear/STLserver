...
//string str{ "POST /2 HTTP/1.1\r\nHost: 42.194.134.131:8085\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: 51\r\n\r\nselectWord=select * from test&indexStart=&indexEnd=" };
...
void startFirstWrite(std::shared_ptr<testSocket> testSock);
...
void startWrite(std::shared_ptr<testSocket> testSock)
{
	if (testSock && testSock->sock && ++i < 10000000)
	{
		boost::asio::async_write(*(testSock->sock), boost::asio::buffer(str), [testSock](const boost::system::error_code &err, const std::size_t size)
		{
			if (!err)
...
void startRead(std::shared_ptr<testSocket> testSock)
{
	if (testSock && testSock->sock)
	{
		testSock->sock->async_read_some(boost::asio::buffer(testSock->arr, 1024), [testSock](const boost::system::error_code &err, std::size_t size)
		{
			if (!err)
			{
				if (std::search(testSock->arr, testSock->arr + size, hello.cbegin(), hello.cend()) != (testSock->arr + size))
					startWrite(testSock);
			}
			else
			{
...
int main()
{
	...
	try
	{
		...
		//vector<std::shared_ptr<boost::asio::ip::tcp::socket>>vec;
		...
		//boost::asio::ip::tcp::endpoint end_point(boost::asio::ip::address::from_string("192.168.80.128"), 8085);
